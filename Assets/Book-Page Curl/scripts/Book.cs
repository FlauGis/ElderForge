...
public class Book : MonoBehaviour {
    ...
    public void AddPageWithDefault(Sprite newPage, Sprite defaultPage)
    {
        Sprite[] updatedPages = new Sprite[bookPages.Length + 2];

        for (int i = 0; i < bookPages.Length; i++)
        {
            updatedPages[i] = bookPages[i];
        }

        updatedPages[bookPages.Length] = newPage;

        updatedPages[bookPages.Length + 1] = defaultPage;

        bookPages = updatedPages;
