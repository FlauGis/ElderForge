...
namespace DevionGames
{
	public class ThirdPersonController : MonoBehaviour
	{
        ...
        private Dictionary<MotionState, bool> isMotionActive = new Dictionary<MotionState, bool>();
        ...
        {
            if (foodSlider.value > 0) return;

            healthTimer += Time.deltaTime;
            if (healthTimer >= healthDecayRate)
            {
                DecreaseHealth(1);
                healthTimer = 0f;
            }
        ...
        {
            m_RawInput = new Vector3(Input.GetAxis(m_HorizontalInput), 0, Input.GetAxis(m_ForwardInput));
            IsAiming = GetAimState();
        }

        private void HandleStamina()
        {
            if (Input.GetKey(KeyCode.LeftShift) && currentStamina > 0 && !Input.GetKey(KeyCode.LeftControl))
            {
            ...
            staminaSlider.value = currentStamina;
            IsStaminaDepleted = currentStamina <= 0;
        }
        // Метод для зменшення їжі
        public void DecreaseFood(int amount)
        {
            foodSlider.value = Mathf.Clamp(foodSlider.value - amount, 0, maxFood);
        }

        // Метод для відновлення їжі
        ...
        {
            if (animator.GetBool("BlockShield"))
			{
                UseStamina(5);
            }
              

            healthSlider.value = Mathf.Clamp(healthSlider.value - amount, 0, maxHealth);
		...
		public void RestoreHealthAndFoood(int health, int food)
		{
            foodSlider.value = Mathf.Clamp(foodSlider.value + food, 0, maxFood);
            healthSlider.value = Mathf.Clamp(healthSlider.value + health, 0, maxHealth);
        }
        public void RestoreHealthIfFoodFull()
        {
            
            if (foodSlider.value > maxFood * 0.75f)
            {
                float restoreAmount = 0.01f;
        ...
        private bool GetAimState()
        {
            ...
            {
                return Input.GetButton(this.m_AimInput);
            }
            if (this.m_AimType.HasFlag<AimType>(AimType.Axis))
            {
            ...
            if (this.m_AimType.HasFlag<AimType>(AimType.Toggle) && Input.GetButtonDown(this.m_AimInput))
            {
                return !IsAiming;
            }
            if (this.m_AimType.HasFlag<AimType>(AimType.Selectable))
            {
                return SelectableObject.current != null;
            }
        ...
        private void HandleMotions()
        {
            ...
            {
                DevionGames.MotionState motion = this.m_Motions[j];


                if (!motion.isActiveAndEnabled || (motion.ConsumeInputOverUI && this.m_GUIClick)) continue;


                bool isMotionAllowed = staminaSlider.value >= 11;

				if (motion.InputName == "Crouch")
				{
					if (inventoryPanel.active == true)
					{
						return;
					}
                    HandleMotionStartStop(motion);
                }
                else if (isMotionAllowed)
                {
                    HandleMotionStartStop(motion);
                }
				else if (!isMotionAllowed && motion.InputName == "Change Speed" && staminaSlider.value != 0)
				{
                    HandleMotionStartStop(motion);
                }
                else
                {
                    HandleMotionStop(motion);
                }
            }
        }

        private void HandleMotionStartStop(DevionGames.MotionState motion)
        {
            if (motion.StartType == StartType.Down && Input.GetButtonDown(motion.InputName))
            {
                if (motion.InputName == "Crouch")
                {
                    if (!isCrouching)
                    ...
                    {
                        isCrouching = false;
                        StartCoroutine(CheckForHeadroomAndStand(motion));
                    }
                }
                else if (!isMotionActive.ContainsKey(motion) || !isMotionActive[motion])
                {

                    this.TryStartMotion(motion);
                    this.m_ToggleState[motion] = true;
                    isMotionActive[motion] = true;
                }
            }
            ...
            if (motion.StopType == StopType.Up && Input.GetButtonUp(motion.InputName))
            {
                if (motion.InputName != "Crouch")
                {
                    ...
                    this.m_ToggleState[motion] = false;
                    isMotionActive[motion] = false;
                }

            }
        }

		private IEnumerator CheckForHeadroomAndStand(DevionGames.MotionState motion)
		{
			float checkHeight = 2f;
			RaycastHit hit;

			if (!Physics.Raycast(transform.position + Vector3.up * 0.5f, Vector3.up, out hit, checkHeight))
			{
                StartCoroutine(DelayedStopMotion(motion, 0.15f));
                isCrouching = false;
			}
            yield return null;
        }

        private void HandleMotionStop(DevionGames.MotionState motion)
        {
            if (motion.InputName == "Climb")
			{
				return;
			}
            if (motion.InputName == "Fall")
            {
                return;
            }

            if (motion.InputName == "Walk")
            {
                if (Input.GetButton(motion.InputName))
                {
                    this.TryStartMotion(motion);
                    isMotionActive[motion] = true;
                }
        ...
        private IEnumerator DelayedStopMotion(DevionGames.MotionState motion, float delay)
        {
        ...
        void RegenerateStamina()
        {
			if (staminaSlider.value < 100)
			{
				...
				{
                    currentStamina += staminaRegenRate/2 * Time.deltaTime;
                    currentStamina = Mathf.Clamp(currentStamina, 0, maxStamina);
                }
            }
        }
